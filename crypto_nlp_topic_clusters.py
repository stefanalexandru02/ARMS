...
def generate_topic_cluster_report(cluster_insights, output_file='topic_cluster_report.html'):
    ...
    for cluster, insights in sorted(cluster_insights.items()):
        html_content += f"""
        <div class="cluster">
            <h2>Cluster {cluster} ({insights['size']} articles)</h2>
...
def visualize_overall_results(df, verb_topic_words, adj_topic_words):
    ...
    # 1. Distribution of articles across clusters
    ...
    # 5. Time series of clusters
    time_series = df.groupby([pd.Grouper(key='date', freq='W'), 'topic_cluster']).size().reset_index(name='count')
    
    fig = px.line(time_series, x='date', y='count', color='topic_cluster', 
                 labels={'date': 'Date', 'count': 'Number of Articles', 'topic_cluster': 'Topic Cluster'},
...
def run_analysis(file_path, n_topics=12, n_clusters=6):
    ...
    return clustered_df, cluster_insights

if __name__ == "__main__":
    # Run the analysis
    result_df, insights = run_analysis('/Users/stefanalexandru/Desktop/ARMS/crypto_nlp_results_intermediate.csv')
